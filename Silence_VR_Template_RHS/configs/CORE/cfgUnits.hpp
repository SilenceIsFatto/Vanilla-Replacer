    class O_T_Soldier_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_rifleman_light";
    }
    class O_T_Soldier_HAT_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_riflemanat";
    }
    class O_T_Soldier_SL_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_squadleader";
    }
    class O_T_Soldier_TL_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_teamleader";
    }
    class O_T_Soldier_LAT_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_rifleman_law";
    }
    class O_T_Pilot_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_airforce_pilot";
    }
    class O_T_Soldier_Repair_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_crewman";
    }
    class O_T_Officer_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_officer";
    }
    class O_T_Soldier_AA_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_stinger";
    }
    class O_T_Soldier_AT_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_javelin";
    }
    class O_T_Soldier_M_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_recon_marpat_wd_marksman_lite";
    }
    class O_T_Helipilot_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_helipilot";
    }
    class O_T_Soldier_GL_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_grenadier";
    }
    class O_T_Engineer_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_engineer";
    }
    class O_T_Soldier_Exp_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_explosives";
    }
    class O_T_Medic_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_rifleman_m4";
    }
    class O_T_Soldier_AR_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_autorifleman";
    }
    class O_T_Soldier_A_F
    {
        factionType = "Silence_VR_RHS_OPF_T_F";
        loadout = "Silence_VR_RHS_rhsusf_usmc_marpat_wd_autorifleman_m249_ass";
    }
    class O_Soldier_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_rifleman";
    }
    class O_Soldier_HAT_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_strelok_rpg_assist";
    }
    class O_Soldier_LAT_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_grenadier_rpg";
    }
    class O_Soldier_AR_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_arifleman_rpk";
    }
    class O_Soldier_A_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_rifleman_alt";
    }
    class O_Soldier_lite_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_rifleman_lite";
    }
    class O_Soldier_SL_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_sergeant";
    }
    class O_Soldier_TL_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_efreitor";
    }
    class O_Pilot_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_pilot_transport_heli";
    }
    class O_soldier_repair_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_driver_armored";
    }
    class O_officer_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_officer_armored";
    }
    class O_Soldier_AA_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_aa";
    }
    class O_Soldier_AT_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_at";
    }
    class O_soldier_M_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_marksman";
    }
    class O_helipilot_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_pilot_tan";
    }
    class O_Soldier_GL_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vmf_emr_grenadier";
    }
    class O_engineer_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vmf_emr_engineer";
    }
    class O_soldier_exp_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vmf_recon_rifleman_asval";
    }
    class O_medic_F
    {
        factionType = "Silence_VR_RHS_OPF_F";
        loadout = "Silence_VR_RHS_rhs_vdv_medic";
    }
    class O_GEN_Soldier_F
    {
        factionType = "Silence_VR_RHS_OPF_GEN_F";
        loadout = "Silence_VR_RHS_O_GEN_Soldier_F";
    }
    class O_GEN_Commander_F
    {
        factionType = "Silence_VR_RHS_OPF_GEN_F";
        loadout = "Silence_VR_RHS_O_GEN_Commander_F";
    }
    class I_C_Soldier_Bandit_7_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_rifleman_akms";
    }
    class I_C_Soldier_Bandit_3_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_machinegunner";
    }
    class I_C_Soldier_Bandit_2_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_grenadier_rpg";
    }
    class I_C_Pilot_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_crew";
    }
    class I_C_Soldier_Bandit_5_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_scout";
    }
    class I_C_Soldier_Bandit_6_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_grenadier";
    }
    class I_C_Soldier_Bandit_1_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_medic";
    }
    class I_C_Soldier_Bandit_8_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_saboteur";
    }
    class I_C_Soldier_Bandit_4_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_rifleman_vz58";
    }
    class I_C_Soldier_Para_7_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_rifleman_mp44";
    }
    class I_C_Soldier_Para_2_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_rifleman_akm";
    }
    class I_C_Helipilot_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_crew";
    }
    class I_C_Soldier_Para_3_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_medic";
    }
    class I_C_Soldier_Para_4_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_machinegunner";
    }
    class I_C_Soldier_Para_6_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_grenadier";
    }
    class I_C_Soldier_Camo_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_warlord";
    }
    class I_C_Soldier_base_unarmed_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_rifleman";
    }
    class I_C_Soldier_Para_8_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_saboteur";
    }
    class I_C_Soldier_Para_1_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_rifleman_aksu";
    }
    class I_C_Soldier_Para_5_F
    {
        factionType = "Silence_VR_RHS_IND_C_F";
        loadout = "Silence_VR_RHS_rhsgref_nat_pmil_grenadier_rpg";
    }
